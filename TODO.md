...
**Status (2026-08-02): orbit drag FIXED.** User testing showed the camera stuck in a fixed view with drags never orbiting. Root cause: `Scene3dWidget::update` pre-seeded `state.last` with the current cursor position on every `CursorMoved` before `handle_event` ran, so every drag delta computed as zero. Fixed by letting `handle_event` own the drag anchor; covered by the `left_drag_orbits_camera` widget test.  Left-drag orbit confirmed working in the GUI. Follow-up (2026-08-02): orbit direction inverted to Blender style (mouse right orbits the view left) in `OrbitCamera::orbit`, and the view gizmo now rotates with the camera — `CameraUniform` carries the `view` matrix, the gizmo pipeline binds the camera UBO, and `gizmo.wgsl` projects the world axes through the view rotation; also fixed the gizmo border shader filling the whole box with the border color. Covered by the `gizmo_axes_follow_camera_orbit` headless test. Grid floor rework (2026-08-02): `grid.wgsl` now uses derivative-based, screen-space-constant line widths (Blender/Golus style) — ~1px lines with coverage AA at any distance, sub-pixel grid levels fade out to kill moiré, and the origin axes are pixel-width too. Verified against the `full_pipeline_with_grid_and_gizmo` render. Orbit floor clamp (2026-08-02): the camera could sink below the Y=0 floor plane, and since `grid.wgsl` ray-casts the floor from the eye, the floor vanished entirely (black void). `OrbitCamera` now clamps pitch after orbit/pan/dolly so the eye always stays `MIN_EYE_HEIGHT` above the plane; covered by `orbit_keeps_eye_above_floor`. REVISITED (2026-08-02): the pitch clamp made bottom views unreachable for floor-hugging models (orbiting under a model 1 unit above the floor requires the eye below the plane). Reverted to a full turntable orbit; instead `grid.wgsl` renders the floor from underneath (dimmed 45% toward the background) when `eye_pos.y < 0`, keeping it as a guide without the black-void sink. Covered by `orbit_reaches_full_pitch_range` and `floor_renders_dimmed_from_below`. Still unverified end-to-end: cursor Grab/Grabbing states, wheel dolly, shift+drag pan, keyboard shortcuts.
